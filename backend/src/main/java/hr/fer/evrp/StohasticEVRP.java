...
import hr.fer.evrp.util.stohastic.distribution.impl.LognormalDistribution;
import hr.fer.evrp.util.stohastic.distribution.impl.NoDistribution;
...
public class StohasticEVRP {
    ...
    static void main(String[] args) {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            usedVehicles.sort(Comparator.comparingInt(Vehicle::getId));

            System.out.println("Number of vehicles: " + usedVehicles.size());
            usedVehicles.forEach(StohasticEVRP::printRoute);
        }
    }
    ...
    private static void printRoute(Vehicle vehicle) {
        ...
        for (int i = 0; i < routeLength; i++) {
            if (i == routeLength - 1) {
                System.out.println(vehicle.getRoute().getLast().getId() + "("
                        + vehicle.getState().getLast() + ")");
            } else {
                System.out.print(vehicle.getRoute().get(i).getId() + "("
                        + vehicle.getState().get(i) + ")" + " -> ");
            }
        }

        double energyUsed = 0;
        for (int i = 1; i < routeLength; i++) {
            energyUsed += Location.distance(
                    vehicle.getRoute().get(i - 1), vehicle.getRoute().get(i));
        }

...
function MapPin({ position, icon, highlighted, onSelect, onMove, onRemove }: MapPinProps) {
  ...
  useEffect(() => {
    ...
    el.style.filter = highlighted
      ? "drop-shadow(0 0 3px white) drop-shadow(0 0 4px black)"
      : "";
  }, [highlighted]);
...
const KEY_MODES: Record<string, PlacementMode> = { v: "select", d: "depot", c: "customer", s: "station", e: "erase" };
...
function DepotMarker() {
  ...
  return (
    <MapPin
      ...
      onSelect={() => { setSelection({ type: "depot", id: "depot" }); setActiveTab("settings"); }}
    ...
    <>
      {customers.map((c) => (
        <MapPin
          ...
          onSelect={() => { setSelection({ type: "customer", id: c.id }); setActiveTab("settings"); }}
    ...
    <>
      {stations.map((s) => (
        <MapPin
          ...
          onSelect={() => { setSelection({ type: "station", id: s.id }); setActiveTab("settings"); }}
